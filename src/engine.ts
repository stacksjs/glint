...
export class GlintEngine {
  ...
  private loadPlugins(): void {
    for (const pluginConfig of this.config.plugins) {
      if (pluginConfig.enabled) {
        try {
          // In a real implementation, dynamically load plugins
  ...
  async lintFiles(patterns?: string[]): Promise<LintResult[]> {
    ...
    try {
      ...
      if (this.config.verbose) {
        console.log(`Found ${filePaths.length} files to lint`)
      }
  ...
  private printMetrics(): void {
    ...
    console.log(`Lint time: ${this.metrics.lintTime.toFixed(2)}ms`)
    console.log(`Format time: ${this.metrics.formatTime.toFixed(2)}ms`)
    console.log(`Files processed: ${this.metrics.filesProcessed}`)
    console.log(`Lines processed: ${this.metrics.linesProcessed}`)
    console.log(`Rules executed: ${this.metrics.rulesExecuted}`)
    ...
    if (this.metrics.linesProcessed > 0) {
      ...
      console.log(`Lines per second: ${linesPerSecond}`)
    }
  }
